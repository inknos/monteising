...
Data Storage can be a problem when dealing with a great amount of data.
We used root data structures to work out the solution. Storing happens two times during a full simulation:
after the ``raw'' collection and after the analysis.\\
All the data are ordered, by temperature, in \texttt{TTree} and by lattice in \texttt{TBranch}. Eanch measurable quantity is a leaf of the branch.
The names of each structures are easy for string comprehension: this way it's easy to read and process previously collected data.\\
